...
export const init = new Command()
  ...
  .action(async (options) => {
    try {
      ...
      logger.info(
        "Project configuration saved to atomicui.components.json",
...
async function promptForProjectDetails(cwd: string) {
  ...
  const { proceed } = await prompts({
    type: "confirm",
    name: "proceed",
    message: `Write configuration to "atomicui.components.json". Proceed?`,
    initial: true,
  });
  ...
  const spinner = ora(`Writing atomicui.components.json...`).start();
  const targetPath = path.resolve(cwd, "atomicui.components.json");
  await fs.writeFile(
    targetPath,
